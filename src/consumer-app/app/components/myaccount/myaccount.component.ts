...
export class MyaccountComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit() {
    if (this.readStripe) {
      ...
      this.elements = this.core.stripe.elements({
        fonts: [
          {
            family: 'Open Sans',
      ...
      this.cardNumber = this.elements.create('cardNumber', { style: elementStyles, classes: elementClasses, });
      this.cardExpiry = this.elements.create('cardExpiry', { style: elementStyles, classes: elementClasses, });
      this.cardCvc = this.elements.create('cardCvc', { style: elementStyles, classes: elementClasses, });
      this.cardZip = this.elements.create('postalCode', { style: elementStyles, classes: elementClasses, placeholder: 'Zipcode', });
  ...
  async onSubmit(form: NgForm) {
    ...
    if (this.getUserInfo === undefined) this.error = "Please login to add new card";
      ...
      const { token, error } = await this.core.stripe.createToken(this.cardNumber);
